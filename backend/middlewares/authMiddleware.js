...
async function handleSpam(req, res, next) {
  try {
    if (!req.headers.cookie) {
      return res.status(400).json({ message: "There isn't any cookie." });
    }

    const tokenCookie = req.headers.cookie.split(";").find((cookie) => cookie.trim().startsWith("token="));
    if (!tokenCookie) {
      return res.status(400).json({ message: "No token for verification." });
    }
...
async function authVerify(req, res, next) {
  try {
    ...
    const decodedToken = await jwt.verify(token, process.env.SECRET_KEY);
    if (!decodedToken) {
      return res.status(400).json({ message: "Please complete your authentication." });
    }
